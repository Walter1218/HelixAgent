...
const scene = {
  ...
  elements: [
    ...
    // 碰撞火花粒子（需要 color 字段）
    { id: 'sparks-1', shape: 'particles', color: '#ffaa00', position: [0, 0, 0], particles: { count: 50, size: 0.1, color: '#ffaa00', opacity: 0.9, spread: 2, speed: 3, lifetime: 1, loop: false, emitterShape: 'sphere', emitterPosition: [0, 0, 0] } },
    { id: 'sparks-2', shape: 'particles', color: '#ff6600', position: [-3.5, 0, -1], particles: { count: 40, size: 0.08, color: '#ff6600', opacity: 0.8, spread: 1.5, speed: 2.5, lifetime: 0.8, loop: false, emitterShape: 'sphere', emitterPosition: [-3.5, 0, -1] } },
    // 爆炸核心
...
async function main() {
  ...
  const frameDir = `${OUTPUT_DIR}/frames/effects-v2`;
  ...
  execSync(`ffmpeg -y -framerate ${FPS} -i ${frameDir}/frame-%04d.png -c:v libx264 -pix_fmt yuv420p -vf 'scale=1280:720' ${OUTPUT_DIR}/three-body-effects-v2.mp4`, { stdio: 'ignore' });
  
  execSync(`ffmpeg -y -i ${OUTPUT_DIR}/three-body-effects-v2.mp4 -vf "select=eq(n\\,0)+eq(n\\,60)+eq(n\\,90)+eq(n\\,120)+eq(n\\,149)" -vsync vfr ${OUTPUT_DIR}/effects-v2-%02d.png`, { stdio: 'ignore' });
  
  console.log('Done: three-body-effects-v2.mp4');
}
