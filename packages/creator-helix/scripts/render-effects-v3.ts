...
const scene = {
  ...
  elements: [
    ...
    // 爆炸光球
    { id: 'fireball-1', shape: 'sphere', color: '#ff6600', position: [0, 0, 0], scale: [0.1, 0.1, 0.1], material: { type: 'basic', emissive: '#ff4400', emissiveIntensity: 3 } },
    { id: 'fireball-2', shape: 'sphere', color: '#ff8800', position: [-3.5, 0, -1], scale: [0.1, 0.1, 0.1], material: { type: 'basic', emissive: '#ff6600', emissiveIntensity: 3 } },
    // 残骸
  ...
  animation: {
    ...
    keyframes: [
      ...
      { time: 7.5, elements: [
        ...
        { id: 'fireball-1', scale: [5, 5, 5] },
        { id: 'fireball-2', scale: [4, 4, 4] },
      ]},
      { time: 10, elements: [
...
async function main() {
  ...
  const frameDir = `${OUTPUT_DIR}/frames/effects-v3`;
  ...
  execSync(`ffmpeg -y -framerate ${FPS} -i ${frameDir}/frame-%04d.png -c:v libx264 -pix_fmt yuv420p -vf 'scale=1280:720' ${OUTPUT_DIR}/three-body-effects-v3.mp4`, { stdio: 'ignore' });
  
  execSync(`ffmpeg -y -i ${OUTPUT_DIR}/three-body-effects-v3.mp4 -vf "select=eq(n\\,0)+eq(n\\,60)+eq(n\\,90)+eq(n\\,120)+eq(n\\,149)" -vsync vfr ${OUTPUT_DIR}/effects-v3-%02d.png`, { stdio: 'ignore' });
  
  console.log('Done: three-body-effects-v3.mp4');
}
