...
async function main() {
  ...
  const frameDir = `${OUTPUT_DIR}/frames/v8`;
  ...
  execSync(`ffmpeg -y -framerate ${FPS} -i ${frameDir}/frame-%04d.png -c:v libx264 -pix_fmt yuv420p -vf 'scale=1280:720' ${OUTPUT_DIR}/three-body-v8.mp4`, { stdio: 'ignore' });
  
  execSync(`ffmpeg -y -i ${OUTPUT_DIR}/three-body-v8.mp4 -vf "select=eq(n\\,0)+eq(n\\,25)+eq(n\\,50)+eq(n\\,75)+eq(n\\,100)+eq(n\\,119)" -vsync vfr ${OUTPUT_DIR}/v8-check-%02d.png`, { stdio: 'ignore' });
  
  console.log('Done: three-body-v8.mp4');
}
