...
const scene = {
  scene: {
    ...
    background: '#020208',
    showGrid: false,
    camera: { position: [45, 25, 45], fov: 60, target: [0, 0, 0] },
    lighting: {
      ambient: { intensity: 0.6, color: '#aabbcc' },
      main: { position: [25, 35, 25], intensity: 2.5 },
  ...
  elements: [
    ...
    { id: 'droplet', shape: 'sphere', color: '#ffffff', position: [25, 5, 15], scale: [0.6, 0.8, 0.6], material: { type: 'physical', metalness: 1, roughness: 0.01, emissive: '#aaddff', emissiveIntensity: 0.8 } },
  ],
  animation: {
    duration: 8, fps: FPS, loop: false,
    keyframes: [
      { time: 0, elements: [{ id: 'droplet', position: [25, 5, 15] }] },
      { time: 2, elements: [{ id: 'droplet', position: [12, 3.5, 8] }] },
    ...
    cameraKeyframes: [
      { time: 0, position: [45, 25, 45], target: [0, 0, 0], fov: 60 },
      { time: 2, position: [30, 18, 30], target: [5, 3, 3], fov: 55 },
      { time: 3.5, position: [18, 12, 20], target: [0, 2.5, 0], fov: 50 },
      { time: 4.5, position: [10, 10, 15], target: [-2, 2, -1], fov: 48 },
      { time: 6, position: [2, 12, 10], target: [-5, 2, -2], fov: 50 },
      { time: 8, position: [-5, 15, 5], target: [-8, 2, -3], fov: 55 },
    ],
  },
  postProcessing: {
...
async function main() {
  ...
  const frameDir = `${OUTPUT_DIR}/frames/v6`;
  ...
  execSync(`ffmpeg -y -framerate ${FPS} -i ${frameDir}/frame-%04d.png -c:v libx264 -pix_fmt yuv420p -vf 'scale=1280:720' ${OUTPUT_DIR}/three-body-v6.mp4`, { stdio: 'ignore' });
  
  execSync(`ffmpeg -y -i ${OUTPUT_DIR}/three-body-v6.mp4 -vf "select=eq(n\\,0)+eq(n\\,25)+eq(n\\,50)+eq(n\\,75)+eq(n\\,100)+eq(n\\,119)" -vsync vfr ${OUTPUT_DIR}/v6-check-%02d.png`, { stdio: 'ignore' });
  
  console.log('Done: three-body-v6.mp4');
}
