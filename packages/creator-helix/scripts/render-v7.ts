...
const scene = {
  scene: {
    ...
    background: '#020208',
    showGrid: false,
    camera: { position: [40, 22, 40], fov: 55, target: [0, 0, 0] },
    lighting: {
  ...
  animation: {
    ...
    cameraKeyframes: [
      { time: 0, position: [40, 22, 40], target: [0, 0, 0], fov: 55 },
      { time: 2, position: [25, 15, 25], target: [5, 3, 3], fov: 50 },
      { time: 3.5, position: [15, 10, 18], target: [0, 2.5, 0], fov: 45 },
      { time: 4.5, position: [8, 8, 12], target: [-2, 2, -1], fov: 42 },
      { time: 6, position: [0, 10, 8], target: [-5, 2, -2], fov: 45 },
      { time: 8, position: [-5, 12, 4], target: [-8, 2, -3], fov: 50 },
    ],
  },
  postProcessing: {
...
async function main() {
  ...
  const frameDir = `${OUTPUT_DIR}/frames/v7`;
  ...
  execSync(`ffmpeg -y -framerate ${FPS} -i ${frameDir}/frame-%04d.png -c:v libx264 -pix_fmt yuv420p -vf 'scale=1280:720' ${OUTPUT_DIR}/three-body-v7.mp4`, { stdio: 'ignore' });
  
  execSync(`ffmpeg -y -i ${OUTPUT_DIR}/three-body-v7.mp4 -vf "select=eq(n\\,0)+eq(n\\,25)+eq(n\\,50)+eq(n\\,75)+eq(n\\,100)+eq(n\\,119)" -vsync vfr ${OUTPUT_DIR}/v7-check-%02d.png`, { stdio: 'ignore' });
  
  console.log('Done: three-body-v7.mp4');
}
