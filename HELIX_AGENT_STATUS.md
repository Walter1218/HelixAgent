...
| ActorWaiter | Phase 3 | ✅ | ✅ tool/actor.ts | 真实 | — | status/wait |
...
| 模块 | 当前状态 | 缺口 |
|------|---------|------|
| Shadow Worktree | `src/shadow-worktree/` 有纯函数实现 | 暂缓 |
...
- **调用位置**：`prompt.ts:1432-1441` — runLoop 结束后调用 `formatTeamByOwnerSession`
- **依赖**：Database.Service
...
- **调用位置**：
  - `processor.ts:471-474` — tool-result 后记录变更文件
  - `prompt.ts:1443-1461` — runLoop 结束后分析 blast radius
- **依赖**：FSUtil.Service
...
- **调用位置**：`prompt.ts:1497-1527` — 完整生命周期管理（startRun + completeRun）
- **依赖**：Database.Service
...
- **调用位置**：`prompt.ts:1125-1136` — budget 检查（`selectTasks` 未调用）
- **依赖**：无外部依赖
...
- **已注册**：OpenSpec / OpenSpecJudge / OpenSpecHook 已在 `app-runtime.ts` 注册
- **调用位置**：`processor.ts:475-494` — tool-result 后调用合规检查
...
| memory_fts | `20260630_add_memory_fts` | 已注册到 migration.gen.ts |

**注意**：迁移文件已存在，但运行时是否已应用取决于用户本地数据库。
...
| Gap | 影响 | 优先级 |
|------|------|--------|
| Scheduler `selectTasks` 未调用 | 任务调度功能不完整 | P2 |
...
| Gap | 影响 | 优先级 |
|------|------|--------|
| Shadow Worktree 未转 Effect Service | 无法注入到主链路 | P3 |
...
| Scheduler `selectTasks` 未调用 | 任务调度功能不完整 | 明确调用点后再接入 |
...
1. **实现 Memory Vector Store**
   - 添加 `memory_vec` 表迁移
   - 实现 VecStore 存储/检索逻辑
   - 集成到 Memory.Service

2. **实现 History Service**
   - 添加 `history_fts` 表
   - 实现 History.Service
   - 集成到 processor.ts

3. **验证现有工具稳定性**
   - memory/history/workflow/screenshot 工具
...
7. 将 Shadow Worktree 转为 Effect Service
8. 实现 Auto-Dev Scheduler 外部脚本
9. 完成 V2 Plugin 系统
